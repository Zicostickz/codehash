...
(define-map license-ownership
  { license-id: uint, owner: principal }
  {
    acquired-at: uint,
...
(define-private (has-valid-license (license-id uint) (user principal))
  (match (map-get? license-ownership { license-id: license-id, owner: user })
    ownership
      (and
...
(define-private (use-license (license-id uint) (user principal))
  (match (map-get? license-ownership { license-id: license-id, owner: user })
    ownership
      (match (get usage-count ownership)
        count 
          (if (> count u0)
            (map-set license-ownership
              { license-id: license-id, owner: user }
...
(define-public (create-license
  ...
  (let
    ...
    (map-set license-ownership
      { license-id: license-id, owner: tx-sender }
      {
...
(define-public (update-license
  ...
  (let
    (
      (license-opt (map-get? licenses { license-id: license-id }))
    )
...
(define-public (purchase-license
  ...
  (let
    ...
    (let
      ...
      (let
        (
          (expiry (match expiration-blocks
...
(define-public (verify-and-use-license (license-id uint) (user principal))
  (begin
    ;; Check if license exists
    (asserts! (map-get? licenses { license-id: license-id }) ERR-LICENSE-NOT-FOUND)
...
(define-public (transfer-license (license-id uint) (recipient principal))
  (let
    (
      (ownership-opt (map-get? license-ownership { license-id: license-id, owner: tx-sender }))
    )
    ...
    (let
      (
        (ownership (unwrap-panic ownership-opt))
      )
      ;; Transfer license to recipient
      (map-set license-ownership
        { license-id: license-id, owner: recipient }
        ownership
      )
      
      ;; Remove license from sender
      (map-delete license-ownership { license-id: license-id, owner: tx-sender })
...
(define-read-only (check-license-ownership (license-id uint) (user principal))
  (is-some (map-get? license-ownership { license-id: license-id, owner: user }))
)

;; Check if a user's license is valid (not expired, has usage left)
(define-read-only (is-license-valid (license-id uint) (user principal))
  (has-valid-license license-id user)
)

;; Get license ownership details
(define-read-only (get-license-ownership (license-id uint) (user principal))
  (map-get? license-ownership { license-id: license-id, owner: user })
)
