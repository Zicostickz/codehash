...
(define-private (calculate-end-block (period-days uint))
  (let ((blocks-per-day u144)) ;; ~10 minute blocks, 6 per hour, 144 per day
...
(define-public (create-proposal 
  ...
  (let ((proposal-id (+ (var-get proposal-count) u1))
    ...
    (asserts! (>= voting-period-days (var-get min-voting-period)) ERR-INVALID-VOTING-PERIOD)
...
(define-public (vote (proposal-id uint) (vote-for bool))
  (let (
    ...
    ;; Record the vote
    (map-set votes {proposal-id: proposal-id, voter: effective-voter} {vote: vote-for, weight: voter-power})
