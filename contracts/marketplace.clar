...
(define-private (check-template-access
    (template-id uint)
    (user principal)
  )
  (match (map-get? purchases {
    ...
    purchase ((match (get-template-by-id template-id)
      template ((let ((licensing-model (get licensing-model template)))
        (cond ;; For one-time purchase, access is perpetual
          ...
          (let ((expiration (default-to u0 (get expiration-block-height purchase))))
          ...
          ;; Fallback for unknown licensing models ERR-INVALID-LICENSING-MODEL
        )
      ))
...
(define-read-only (has-template-access
    (template-id uint)
    (user principal)
  )
      ...
      (match (transfer-stx price tx-sender owner)
        success-tx (
          ...
          success-fee (
            ...
            LICENSE-USAGE-BASED ((let ((limit (get usage-limit template)))
              (map-set purchases {
                template-id: template-id,
                user: tx-sender,
              } {
    ...
    template (
      ...
      (match (stx-transfer? price tx-sender (as-contract tx-sender))
        success-tx (
          ...
          (map-set escrow-payments { escrow-id: escrow-id } {
          template-id: template-id,
          buyer: tx-sender,
          seller: owner,
          amount: price,
      ...
      (match (map-get? templates { template-id: template-id })
        template ((let ((licensing-model (get licensing-model template)))
          ...
          (match (as-contract (stx-transfer? seller-amount tx-sender seller))
            success-seller ((match (as-contract (stx-transfer? platform-fee tx-sender (var-get platform-address)))
              success-fee (
                ;; Track platform revenue
                (add-platform-revenue platform-fee)
      ...
      (match (transfer-stx price tx-sender owner)
        success-tx (
          ...
          success-fee (
            ...
            existing-purchase
            ((let (
                (current-expiration (default-to block-height
                  (get expiration-block-height existing-purchase)
                ))
...
(define-public (record-template-usage
    (template-id uint)
    (user principal)
  )
  (match (map-get? templates { template-id: template-id })
    template (
      ...
      purchase ((let (
        ...
        (map-set purchases {
          ...
          (merge purchase { usage-count: (+ usage-count u1) })
        )
        (ok (+ usage-count u1))
      ))
      ERR-ACCESS-DENIED
