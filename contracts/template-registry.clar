...
(define-map template-versions
  { template-id: uint, version: (string-ascii MAX-VERSION-LENGTH) }
  {
...
(define-private (is-owner? (template-id uint))
  (let (
    (template-data (map-get? templates { template-id: template-id }))
  )
    (and
...
(define-private (version-exists? (template-id uint) (version (string-ascii MAX-VERSION-LENGTH)))
  (is-some (map-get? template-versions { template-id: template-id, version: version }))
)
...
(define-private (add-version-to-list (template-id uint) (version (string-ascii MAX-VERSION-LENGTH)))
  (let (
    (current-list (default-to { versions: (list) } (map-get? template-version-list { template-id: template-id })))
    (new-list (unwrap-panic (as-max-len? (append (get versions current-list) version) u20)))
  )
    (map-set template-version-list 
...
(define-read-only (get-template-version (template-id uint) (version (string-ascii MAX-VERSION-LENGTH)))
...
(define-read-only (is-template-owner (template-id uint) (owner principal))
  (let (
    (template-data (map-get? templates { template-id: template-id }))
  )
    (match template-data
...
(define-public (register-template
  ...
  (let (
    ...
    (map-set template-compatibility
      { template-id: new-template-id }
      {
...
(define-public (update-template-compatibility
  ...
  (begin
    ...
    (map-set templates
      { template-id: template-id }
      (merge (unwrap-panic (map-get? templates { template-id: template-id }))
        { last-updated: block-height }
      )
    )
...
(define-public (publish-template-version
  ...
  (begin
    ...
    (asserts! (not (version-exists? template-id version)) ERR-VERSION-ALREADY-EXISTS)
...
(define-public (deprecate-template-version
  ...
  (let (
    (version-data (map-get? template-versions { template-id: template-id, version: version }))
  )
    ...
    (map-set template-versions
      { template-id: template-id, version: version }
      (merge (unwrap-panic version-data)
        { is-deprecated: true }
      )
    )
