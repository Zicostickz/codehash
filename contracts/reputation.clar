...
(define-private (calculate-average-rating (total-rating uint) (rating-count uint))
...
(define-private (update-developer-reputation (developer principal) (rating uint))
  (let (
    (current-reputation (default-to
      { total-rating: u0, rating-count: u0, total-templates: u0, total-sales: u0 }
      (map-get? developer-reputations { developer: developer })
    ))
  )
    (map-set developer-reputations
      { developer: developer }
      {
...
(define-private (update-template-reputation (template-id uint) (rating uint))
  (let (
    (current-reputation (default-to
      { total-rating: u0, rating-count: u0, total-purchases: u0, total-deployments: u0 }
      (map-get? template-reputations { template-id: template-id })
    ))
  )
    (map-set template-reputations
...
(define-private (has-purchased-template (template-id uint) (user principal))
  (default-to false (get purchased (map-get? template-purchases { template-id: template-id, user: user })))
)

;; Public functions

;; Register a template purchase - can only be called by the marketplace contract
(define-public (register-purchase (template-id uint) (buyer principal) (seller principal))
  (begin
...
(define-public (register-template (template-id uint) (creator principal))
  (begin
    ;; Only marketplace contract can call this function
    (asserts! (is-eq tx-sender MARKETPLACE-CONTRACT) ERR-MARKETPLACE-ONLY)
...
(define-public (submit-review (template-id uint) (rating uint) (review-text (string-utf8 500)) (template-owner principal))
  (begin
    ...
    (asserts! (is-none (map-get? template-reviews { template-id: template-id, reviewer: tx-sender })) ERR-ALREADY-REVIEWED)
...
(define-public (update-review (template-id uint) (rating uint) (review-text (string-utf8 500)))
  (let (
    (existing-review (map-get? template-reviews { template-id: template-id, reviewer: tx-sender }))
  )
    ...
    (let (
      ...
      (template-rep (unwrap! (map-get? template-reputations { template-id: template-id }) ERR-TEMPLATE-NOT-FOUND))
      ...
      (let (
        ...
        (developer-rep (unwrap! (map-get? developer-reputations { developer: template-owner }) ERR-USER-NOT-FOUND))
      )
        ;; Update the review
        (map-set template-reviews
          { template-id: template-id, reviewer: tx-sender }
...
(define-read-only (get-template-reputation (template-id uint))
  (let (
    (reputation (map-get? template-reputations { template-id: template-id }))
  )
    (if (is-some reputation)
      (let (
        (unwrapped-rep (unwrap-panic reputation))
      )
        (ok {
          average-rating: (calculate-average-rating 
            (get total-rating unwrapped-rep)
...
(define-read-only (get-developer-reputation (developer principal))
  (let (
    (reputation (map-get? developer-reputations { developer: developer }))
  )
    (if (is-some reputation)
      (let (
        (unwrapped-rep (unwrap-panic reputation))
...
(define-read-only (get-review (template-id uint) (reviewer principal))
  (let (
    (review (map-get? template-reviews { template-id: template-id, reviewer: reviewer }))
  )
    (if (is-some review)
...
(define-read-only (has-user-purchased (template-id uint) (user principal))
  (ok (has-purchased-template template-id user))
)

;; Check if a user has already reviewed a template
(define-read-only (has-user-reviewed (template-id uint) (user principal))
  (ok (is-some (map-get? template-reviews { template-id: template-id, reviewer: user })))
)